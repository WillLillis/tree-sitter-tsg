LANGUAGE_NAME := tree-sitter-tsg
HOMEPAGE_URL := https://github.com/WillLillis/tree-sitter-tsg
DESCRIPTION := A tree-sitter grammar for TSG, the native grammar DSL for tree-sitter
VERSION := 0.1.0
...
$(SRC_DIR)/grammar.json: grammar.tsg
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/tsg '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/tsg
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/tsg
